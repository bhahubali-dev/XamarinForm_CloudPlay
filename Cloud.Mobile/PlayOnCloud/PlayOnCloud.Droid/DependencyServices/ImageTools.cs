	...
	public class ImageTools : IImageTools
	{
		public void GetImageSize(byte[] data, out int width, out int height)
		{
			...
			if ((data != null) && (data.Length > 0))
			{
				...
				{
					Log.Error(nameof(ImageTools), "ERROR: ImageToolsService.GetImageSize: " + ex);
				}
			}
		}
		...
		public async Task<byte[]> GetImageFromCache(string key)
		{
			...
			{
				Log.Error(nameof(ImageTools), "ERROR: ImageToolsService.GetImageFromCache: " + ex);
			}
		...
		public Task PreloadUrl(string url)
		{
			...
			{
				Log.Error(nameof(ImageTools), "ERROR: ImageToolsService.PreloadUrl: " + ex);
			}
